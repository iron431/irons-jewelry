...
public class LootInjectionHandler {
    ...
    private static Map<Predicate<Item>, Integer> createGearscoreMap() {
        ...
        map.put(is(JewelryModTags.LOOT_HANDLER_VERY_HIGH_GEARSCORE), 60);
        map.put(item -> item instanceof ArmorItem || item instanceof SwordItem, 100);
        return map;
    }
    ...
    public static void onLootTableLoad(LootTableLoadEvent event) {
        ...
        //TODO: somehow limit to chest/vaults/whatever only. has something to do with the loot context param registered name but im not entirely sure atp
            ...
            for (LootPoolEntryContainer entry : pool.entries) {
                if (entry instanceof LootItem itemHolder) {
                    ...
                    if (item instanceof CurioBaseItem) {
                        ...
                        IronsJewelry.LOGGER.debug("LootTable {} already contains jewelry, skipping", event.getName());
                        return;
                    }
                    gearscore += gearscoreFor(item.value());
                    //todo: remove debuggers
                    if (gearscoreFor(item.value()) > 0) {
                        IronsJewelry.LOGGER.debug("LootTable {} item {} score {}", event.getName(), item.value(), gearscoreFor(item.value()));
                    }
                }
            }
        }
        if (gearscore >= GEARSCORE_THRESHOLD) {
            float chance = Mth.clampedLerp(0.05f, 0.75f, (gearscore - GEARSCORE_THRESHOLD) / 500f);
